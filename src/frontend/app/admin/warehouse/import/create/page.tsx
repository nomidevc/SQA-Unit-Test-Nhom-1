      ...
      if (isExcel) {
        ...
        for (let i = productStartIndex; i < jsonData.length; i++) {
          ...
          if (sku) {
            ...
            // Start new product
            if (row.length >= 4) {
              // Column mapping có 3 format:
              // Format 1: 0-SKU, 1-Tên, 2-Số lượng, 3-Giá, 4-Bảo hành, 5-Thông số (text/JSON), 6-Ghi chú
              // Format 2: 0-SKU, 1-Tên, 2-Số lượng, 3-Giá, 4-Bảo hành, 5-TS1Tên, 6-TS1Giá trị, 7-TS2Tên, 8-TS2Giá trị, ..., cuối-Ghi chú
              // Format 3 (merged): 0-SKU, 1-Tên, 2-Số lượng, 3-Giá, 4-Bảo hành, 5-TSName, 6-TSValue, 7-Ghi chú (sub-rows have empty SKU)
              ...
              if (row.length >= 7) {
                ...
                // Check for Format 3 (merged cells) or Format 2 (multi-column)
                if (col5 && col5.length < 30 && col6 && !col5.includes(':') && !col5.includes('{')) {
                  // Could be Format 2 or Format 3
                  // Format 3: col7 is note (short, or typical note text)
                  // Format 2: col7 is another tech spec name
            ...
            if (specName && specValue) {
              // Remove empty placeholder if this is first real spec
